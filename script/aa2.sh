...
filesrc location="/usr/share/somapp/movies/walking-people.nv12.30fps.1080p.h264" \
  ...
  ! tee name=t0 t0.src_0 ! queue \
    ! ivas_xm2m kconfig="./ped_pp.json" ! \
    ! ivas_xfilter name=refinedet kernels-config="./refinedet.json" ! queue \
    ! ivas_xfilter name=crop      kernels-config="./crop.json" ! queue \
    ! ivas_xfilter kernels-config="./reid.json" ! queue \
    ! scalem0.sink_master ivas_xmetaaffixer name=scalem0 scalem0.src_master \
    ! fakesink \
  t0.src_1 ! queue ! \
    scalem0.sink_slave_0 scalem0.src_slave_0 ! queue \
    ! ivas_xfilter kernels-config="./draw_reid.json" ! queue \
